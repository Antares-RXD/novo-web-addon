...
export class Wallet {
  private readonly privateKey: novo.PrivateKey;
  private publicKey: novo.PublicKey;
  ...
  constructor(
    ...
    if (privateKey && !novo.PrivateKey.isValid(privateKey.toString("hex"))) {
      throw new Error(
        "Private key does not satisfy the curve requirements (ie. it is invalid)"
      );
    }
    if (privateKey) {
      this.privateKey = novo.PrivateKey.fromBuffer(privateKey, network);
    }

    if (publicKey && !novo.PublicKey.isValid(publicKey.toString("hex"))) {
      throw new Error("Invalid public key");
    }
    if (publicKey) {
      this.publicKey = novo.PublicKey.fromBuffer(publicKey);
    }
  }
   ...
   *
   */
  public static generate(network?: "mainnet" | "testnet"): Wallet {
    return new Wallet(novo.PrivateKey.fromRandom(network).toBuffer());
  }
  ...
  public static generateVanityAddress(
    ...
    while (true) {
      const privateKey = novo.PrivateKey.fromRandom(network);
      const address = privateKey.toAddress(network).toString();

      if (pattern.test(address)) {
        return new Wallet(privateKey.toBuffer());
      }
    }
  }
  ...
  public static async fromV3(
    ...
    const mac = novo.crypto.Hash.sha256(
  ...
  private get pubKey(): Buffer {
    ...
    return this.privateKey.publicKey.toBuffer();
  }
  ...
  public getAddress(): Buffer {
    if (this.privateKey) {
      return this.privateKey.toAddress(this.network).toBuffer();
    }
    return this.publicKey.toAddress(this.network).toBuffer();
  }
